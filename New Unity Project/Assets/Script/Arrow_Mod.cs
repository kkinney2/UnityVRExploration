...
{
    //-------------------------------------------------------------------------
    public class Arrow_Mod : MonoBehaviour
    {
        ...
        // MOD_AREA
        public GameObject prefab_SplitterArrow;
        public bool isSplitterArrow = false;
        ...
        void OnCollisionEnter(Collision collision)
        {
            if (inFlight)
            {
                ...
                if (isSplitterArrow)
                {
                    ...
                    GameObject splitterArrow = Instantiate(prefab_SplitterArrow);
                    splitterArrow.transform.position = transform.position;
                    splitterArrow.transform.rotation = transform.rotation;

                    Arrow_Mod[] arrows = splitterArrow.GetComponentsInChildren<Arrow_Mod>();

                    foreach (Arrow_Mod arrow in arrows)
                    {
                        ...
                        arrow.arrowHeadRB.AddForce(gameObject.transform.forward * GetComponent<Rigidbody>().velocity.magnitude, ForceMode.VelocityChange);
                        arrow.arrowHeadRB.AddTorque(gameObject.transform.forward * 10);

                        arrow.ArrowReleased(GetComponent<Rigidbody>().velocity.magnitude);

                        arrow.arrowHeadRB.velocity += reflfectDir * 0.25f;
                        arrow.shaftRB.velocity += reflfectDir * 0.25f;
                    }
                    #region Original Arrow Code
                    /*
                    Arrow arrow = currentArrow.GetComponent<Arrow>();
                    arrow.shaftRB.isKinematic = false;
                    arrow.shaftRB.useGravity = true;
